  ...
  <div>
    <Modal v-model="visible"
      ...
      <Form class="panel-form"
            ...
            :label-width="120"
            >
        <FormItem label="账号：" prop="login"><i-input v-model.trim="detail.login" :disabled="!!id" placeholder="请输入账号" type="text"></i-input></FormItem>
        ...
        <FormItem label="贴牌商名称：" prop="name"><i-input v-model.trim="detail.name" placeholder="请输入贴牌商名称" type="text"></i-input></FormItem>
        ...
        <FormItem label="迹点：" prop="moneyInit"><InputNumber :disabled="!!id" v-model="detail.moneyInit" :min="0" class="wp100"></InputNumber></FormItem>
        <FormItem label="地址：">
           <Row
              class="">
              <i-col span="8">
...
export default {
  ...
  methods: {
    loadDetail () {
      ...
      oemApi.getOemDetail(this.id).then(data => {
        this.isLoadSet = true
        this.detail = {...data}
      })
    },
    async handlerSave () {
      ...
      if (id) {
        data.id = id
        data.id2 = id2
        await oemApi.updateOem(data)
      } else {
        data.password = password
        await oemApi.addOem(data)
      }
