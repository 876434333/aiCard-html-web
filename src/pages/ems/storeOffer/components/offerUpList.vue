<template>
  <div>
    ...
    <div style="margin-top:20px">
      <Button @click="handlerMoreUnderCarriage">批量下架</Button>
    </div>
...
export default {
          ...
          render: (h, { row, column, index }) => {
            return h('div', [
              h('Button', {
                ...
                on: {
                  click: () => {
                    this.handlerUnderCarriage(row.id)
                  }
                }
              }, '下架'),
              h('Button', {
  ...
  created() {
    this.$onBus('offerUpListChange', target => {
  ...
  methods: {
    ...
    selectRowAll(selection) {
      ...
      for (var i = 0; i < selection.length; i++) {
        this.idList.push(selection[i].id)
      }
    },
