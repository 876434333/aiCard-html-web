  ...
  <div>
    <Modal v-model="visible"
      ...
      <Form class="panel-form"
        ...
        <FormItem label="地区总代理商名称：" prop="name"><i-input v-model.trim="detail.name" placeholder="请输入贴牌商名称" type="text"></i-input></FormItem>
        ...
        <FormItem label="迹点：" prop="moneyInit"><InputNumber v-model="detail.moneyInit" :disabled="!!id" :min="0" :max="99999999" class="wp100"></InputNumber></FormItem>
        <FormItem label="地址：">
           <Row
              class="">
              <i-col span="8">
...
import {
  accountValidator,
  passwordValidator,
  mobileOrTelphoneValidator
} from '@/assets/js/asyncValidator/validators'
export default {
  props: {
    value: false,
    id: ''
  ...
  methods: {
    loadDetail () {
      ...
      regionalAgentApi.getAgentDetail(this.id).then(data => {
        this.isLoadSet = true
        this.detail = data
      })
    },
    async handlerSave () {
      ...
      if (id) {
        data.id = id
        data.id2 = id2
        await regionalAgentApi.updateAgent(data)
      } else {
        data.password = password
        await regionalAgentApi.addAgent(data)
      }
